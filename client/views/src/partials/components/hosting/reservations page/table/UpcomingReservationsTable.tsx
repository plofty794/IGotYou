...
const columns: ColumnDef<TUpcomingReservations>[] = [
  {
    accessorKey: "status",
    header: ({ column }) => {
      return (
    ...
    cell: ({ row }) => (
      <Badge
        className={`rounded-full font-bold uppercase ${
          row.original.status === "cancelled" ? "text-red-600" : "text-blue-600"
        } `}
        variant={"outline"}
      >
        {row.original.status}
...
function UpcomingReservationsTable({
  data,
}: {
  data: TUpcomingReservations[];
}) {
...
export default UpcomingReservationsTable;
